...
The `Withdraw` instruction transfers SOL from a stake account to a recipient.
The amount that can be withdrawn depends on the stake account's state and lockup status.
