...
async function withdraw(
    ...
    const createStakeIx = SystemProgram.createAccount({
        ...
        newAccountPubkey: userStake.publicKey,
        lamports: rent,
        space: 200,
        programId: StakeProgram.programId,
    });
    
    // Choose a validator to withdraw from
    const validatorList = await getValidatorList(
        connection,
        poolData.account.data.validatorList
    );
    const validator = validatorList.validators[0]; // or preferred
    ...
    console.log('New stake account:', userStake.publicKey.toBase58());
}
```
